...
class Paths:
    def __init__(self, output_dir):
        ...
        self.save_marked_dir = output_dir.joinpath("HinhAnhKiemTra")
        self.results_dir = output_dir.joinpath("KetQuaKiemTra")
...
def setup_dirs_for_paths(paths):
    logger.info("Đang kiểm tra đường dẫn...")
    for save_output_dir in [paths.save_marked_dir]:
        if not os.path.exists(save_output_dir):
            logger.info(f"Tạo kết quả : {save_output_dir}")
...
def setup_outputs_for_template(paths, template):
    ...
    logger.info("Kiểm tra các tập tin...")
    ...
    for file_key, file_name in ns.filesMap.items():
        if not os.path.exists(file_name):
            logger.info(f"Xuất kết quả ra tập tin: '{file_name}'")
            ...
            logger.info(f"Hiện tại: thêm vào '{file_name}'")
