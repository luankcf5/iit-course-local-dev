...
class ImageInstanceOps:
    ...
    def read_omr_response(self, template, image, name, save_dir=None):
        ...
        try:
            ...
            logger.info(
                f"Ngưỡng:\tglobal_thr: {round(global_thr, 2)} \tglobal_std_THR: {round(global_std_thresh, 2)}\t{'(Looks like a Xeroxed OMR)' if (global_thr == 255) else ''}"
            )
            # plt.show()
            # hist = getPlotImg()
            # InteractionUtils.show("StdHist", hist, 0, 1,config=config)
            ...
            for field_block in template.field_blocks:
                ...
                for field_block_bubbles in field_block.traverse_bubbles:
                    ...
                    per_q_strip_threshold = self.get_local_threshold(
                        ...
                        global_thr,
                        no_outliers,
                        f"Biểu đồ cường độ trung bình cho {key}.{field_block_bubbles[0].field_label}.{block_q_strip_no}",
