...
class AnswerMatcher:
    ...
    def get_section_explanation(self):
        ...
        if answer_type in ["standard", "multiple-correct"]:
            ...
            return f"Tuỳ chỉnh: {self.marking}"
    ...
    def parse_scheme_marking(self, marking):
        ...
        for verdict_type in MARKING_VERDICT_TYPES:
            ...
            if (
                ...
                logger.warning(
                    f"Tìm thấy dấu hiệu tích cực({round(verdict_marking, 2)}) cho câu trả lời sai trong lược đồ '{self.section_key}'. Đối với các phần Tiền thưởng, hãy thêm tiền tố 'BONUS_' vào chúng."
                )
    ...
    def __init__(self, curr_dir, evaluation_path, template, tuning_config):
        ...
        if source_type == "csv":
            ...
            if not os.path.exists(csv_path):
                logger.warning(f"Khóa trả lời csv không tồn tại trong: '{csv_path}'.")
            ...
            if os.path.exists(csv_path):
                ...
                raise Exception(f"Không tìm thấy khóa trả lời csv tại '{csv_path}'")
            else:
                image_path = str(curr_dir.joinpath(answer_key_image_path))
                if not os.path.exists(image_path):
                    raise Exception(f"Không tìm thấy hình ảnh đáp án tại '{image_path}'")

                # self.exclude_files.append(image_path)

                logger.debug(
                    f"Đang cố gắng tạo đáp án từ hình ảnh: '{image_path}'"
                )
                ...
                if in_omr is None:
                    raise Exception(
                        f"Không thể đọc đáp án từ hình ảnh {image_path}"
                    )
                (
                    response_dict,
                    _final_marked,
                ...
                if "questions_in_order" in options:
                    ...
                    if len(empty_answered_questions) > 0:
                        logger.error(
                            f"Tìm thấy câu trả lời trống cho câu hỏi: {empty_answered_questions}, giá trị trống được sử dụng: '{empty_val}'"
                        )
                        raise Exception(
                            f"Tìm thấy câu trả lời trống trong tập tin '{image_path}'. Vui lòng kiểm tra lại mẫu của bạn ở chế độ --setLayout."
                        )
                else:
                    logger.warning(
